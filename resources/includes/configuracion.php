<section class="container mt-5 seccion" id="configuracion">
	<div class="row d-flex align-items-center alert alert-info text-center mt-5">
    	<p class="my-auto">En los siguientes inputs vas a poder configurar aspectos importantes para el funcionamiento de la app</p>
	</div>
	<div class="row my-4">
		<div class="col-12 mt-5 input-group mb-3">
			<form class="d-flex w-75" action="./resources/includes/globales.php" method="POST">
				<label for="diasClases" class="fw-medium w-75 text-center align-middle lh-lg">Dias totales de clases</label>
				<input type="number" name="diasClases" id="diasClases" class="form-control ms-2" required>
				<button type="submit" class="btn btn-outline-success ms-2">Aplicar</button>
			</form>
		</div>
	</div>
	<div class="row my-4">
		<div class="col-12 mt-5 input-group mb-3">
			<form class="d-flex w-75" action="./resources/includes/globales.php" method="POST">
				<label for="porcentajePromocion" class="fw-medium w-75 text-center align-middle lh-lg">Porcentaje para promocionar</label>
				<input type="number" name="porcentajePromocion" id="porcentajePromocion" class="form-control ms-2" required>
				<button type="submit" class="btn btn-outline-success ms-2">Aplicar</button>
			</form>
		</div>
	</div>
	<div class="row my-4">
		<div class="col-12 mt-5 input-group mb-3">
			<form class="d-flex w-75" action="./resources/includes/globales.php" method="POST">
				<label for="porcentajeRegularizar" class="fw-medium w-75 text-center align-middle lh-lg">Porcentaje para regularizar</label>
				<input type="number" name="porcentajeRegularizar" id="porcentajeRegularizar" class="form-control ms-2" required>
				<button type="submit" class="btn btn-outline-success ms-2">Aplicar</button>
			</form>
		</div>
	</div>
</section>